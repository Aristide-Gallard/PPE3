...
FROM modele rp WHERE rp.id_MODELE = :id_modele";
	...
	{
		$req = "DELETE FROM associe WHERE Id_MODELE = :ID_MODELE";
		$res = PdoMudry::$monPdo->prepare($req);
		$res->bindValue("ID_MODELE", $id);
		$res->execute();
 ...
 */
 ...
 public static function modifMouvement($nbPlace,$distance,$heureD,$duree,$heureA,$Id_AEROPORT,$Id_AEROPORT_1,$Id_AVION ){
    ...
    {
        $req = "SELECT commercial.id_PERSONNEL AS num, tel FROM personnel INNER JOIN commercial ON personnel.Id_PERSONNEL = commercial.Id_PERSONNEL  WHERE commercial.id_PERSONNEL = :num";
        $res = PdoMudry::$monPdo->prepare($req);
        $res->bindValue(":num", $num, PDO::PARAM_INT);
        $res->execute();
        $ligne = $res->fetch(PDO::FETCH_ASSOC);
    ...
    public function modificationPersonnelC($tel, $num, $langues)
{
    ...
    public function ajouterLangueAuPersonnel($num, $langueId)
    {
        ...
        $query = 'INSERT INTO parle ( Id_LANGUE) VALUES ( :langueId)';
